...
with st.sidebar:
    ...
    current_label = next(label for label, value in view_labels.items() if value == st.session_state.current_view)
    chosen = st.radio("Navigation", list(view_labels), index=list(view_labels).index(current_label), label_visibility="collapsed")
    st.session_state.current_view = view_labels[chosen]
    ...
    if st.button("Refresh data", width="stretch"):
        st.cache_data.clear()
        st.rerun()
    st.markdown(f'<div class="small-muted">Cached for up to {max(1, CACHE_TTL_SECONDS // 3600)} hours</div>', unsafe_allow_html=True)
